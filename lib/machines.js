...
xx.machineTypeFromSpec = function (machineTypeTransitionSpec, machineTypeName) {
    ...
    constructor.prototype.transition = function (newState) {
        if (! this.transitions[newState])
            throw new SyntaxError('attempt to transition machine ' + xx.formatMachine(this) + ' to non-existent state ' + newState)
        if (! this.transitions[this.state][newState])
            throw new SyntaxError('invalid transition in machine ' + xx.formatMachine(this)
                                   + ': ' + this.state + '->' + newState)
        this.state = newState
    }
