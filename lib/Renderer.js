...
Renderer.testRenderer = function (chunks, i) {
    ...
    for (var t in chunk.transitions) {
        s += 'TRANSITION ' + t.machineType + '(' + t.machineName + ')->' + t.to
        s += '\n    WITH DATA\n' + formatKeyValuePairs(t.keyValuePairs, 4)
    }
...
Renderer.prototype.renderFileSync = function(options) {
    ...
    var chunks = Renderer.chunksFromString(options.findTransitionChunk || this.findTransitionChunk, src)
...
Renderer.prototype.stringsFromChunks = function (chunks) {
    ...
    for (var i = 0; i < chunks.length; i++) {
        ...
        for (var t in chunk.transitions) {
            ...
            if (! this.machines[t])
                qqutil.throwTypeError('attempt to transition a machine of unknown type')
            ...
            for (var n in chunk.transitions[t]) {
                ...
                this.machines[t][n].transition(t.to)
            }

            strings[i] = this.chunkRenderer(chunks, i)
        }
    }
    for (var t in this.machines) {
        for (var n in this.machines[t]) {
...
// transition chunk.
Renderer.chunksFromString = function (findTransitionChunk, src) {
